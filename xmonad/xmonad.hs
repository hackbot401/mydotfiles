...
myKeys =
     ...
     , ("<XF86Explorer> r g", runOrRaisePrompt            myXPConfig)   -- Run or go to a gui application
       ...
       -- Dmenu
     , ("M-S-<Return>", spawn "dmenu_run -i -p \"Run: \"")             -- Demenu recency (adapted from Manjaro i3)
     ...
     , ("M-y", spawn ("spotify"))                                     -- MS teams (thanks work!!!)
...
myWorkspaces = clickable . map xmobarEscape
               $ ["main", "web", "music", "edit", "files", "chat", "write", "virt", "watch"]
  where
